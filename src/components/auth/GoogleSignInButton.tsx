...
export const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  const handleCredential = async (credential: string) => {
    setIsLoading(true);
    try {
      const user = await authService.handleGoogleCredential(credential);
      if (onSuccess) {
        onSuccess(user);
      } else {
    ...
    const initGsi = () => {
      if (window.google?.accounts?.id && googleBtnContainerRef.current) {
        try {
          ...
          // Render Google's official GIS button
          ...
          console.warn('Google accounts.id render error:', e);
          return false;
        }
      }
      return false;
  ...
  return (
    <div className={`w-full space-y-2 ${className}`}>
      {/* Container where official Google button renders */}
      <div 
        ref={googleBtnContainerRef} 
        className={`w-full flex justify-center overflow-hidden rounded-lg min-h-[44px] ${!gisLoaded ? 'hidden' : ''}`}
      />
